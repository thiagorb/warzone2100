...
std::map<uint32_t, int32_t> grid_allocation::allocator::calculate_offsets(int32_t space)
{
	...
	for (size_t i = 0; i < sizes.size(); i++)
	{
		...
		offsets[steps_lookup[i + 1]] = current_offset;
	}
...
void grid_allocation::allocator::expand(std::vector<int32_t>& sizes, int32_t space, size_t start, size_t end)
{
	...
	for (size_t i = start; i < end; i++)
	{
		if (expandable_map[i])
		{
			slots_to_expand += steps_lookup[i + 1] - steps_lookup[i];
		}
	}
	...
	if (!has_expandable)
	{
		slots_to_expand = steps_lookup[end] - steps_lookup[start];
	}
	...
	for (size_t i = start; i < end; i++)
	{
		...
		{
			auto span = steps_lookup[i + 1] - steps_lookup[i];
...
void grid_allocation::allocator::initialize_steps(const std::vector<grid_allocation::item> &items)
{
	...
	{
		steps.insert(item.start());
		steps.insert(item.end());
	}

	slots_size = steps.size() - 1;
	steps_lookup.reserve(steps.size());
	for (auto step: steps)
	{
		steps_index[step] = steps_lookup.size();
		steps_lookup.push_back(step);
	}
}
